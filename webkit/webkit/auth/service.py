...
from webkit.cache.redisclient import BaseRedisClient
...
class JWTService(BaseJWTService):
    ...
    async def _get_db_data(self, validated_data: TokenData) -> str:
        ...
        async with self._backend.lock(key):
            refresh_token = await self._backend.redis.get(key)
    ...
    async def _save_db_data(self, refresh_data: TokenData) -> None:
        ...
        async with self._backend.lock(key):
            await self._backend.redis.set(key, val, ex=self.refresh_token_expire_time)
